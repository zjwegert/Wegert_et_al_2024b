...
function main(ranks,params)
  ...
  Bᴴ,DBᴴ = bulk_modulus_3d(Cᴴ,eᴴ,κᴴ,DCᴴ,Deᴴ,Dκᴴ)
...
@main function run(;
    ...
    write_dir::String=ENV["PROJECT_DIR"]*"/results/3d_n=$(n)_o=$(order)_max_Bh_st_Vol=$(vol)_dh=$(dh)_gam=$(gamma)_amin=$(amin)_alph=$(alpha_coeff)_ls=$(line_search)$(write_dir_suffix)/",
    iter_mod::Int=10,
    reinit_mod::Int=5,
