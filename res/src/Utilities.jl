...
function build_history_plots!(data;
  ...
  for ID in IDs
    ...
    with_theme(theme_latexfonts(), palette=(color=colors,markercolor=color,patchcolor=color)) do
      ...
      hidedecorations!(axes)
      hidespines!(axes)
      ax1 = Axis(f[1, 3], yticklabelcolor = colors[1], ylabelcolor = colors[1], xlabel = L"\text{Iteration}", ylabel=L"$\bar{B}_{\mathrm{%$(loc_data.Objective[1][4:end])}}(\Omega)$",
      ...
      req_dh = cfmt("%.$(nflts)f",loc_data.Req_dh[1]*10)*"\\times10^{-10}";
      lines!(ax2,first(loc_data.History).Vol,linestyle=:dot,color=colors[2],linewidth = 3,label=L"""\mathrm{Vol}(\Omega)-%$(req_vol)""")
      lines!(ax2,first(loc_data.History).dh,linestyle=:dash,color=colors[2],linewidth = 3,label=L"\bar{d}_h(\Omega)-%$(req_dh)")
...
function build_history_plots_dh!(data;
  ...
  for ID in IDs
    ...
    with_theme(theme_latexfonts(), palette=(color=colors,markercolor=color,patchcolor=color)) do
      ...
      hidedecorations!(axes)
      hidespines!(axes)
      req_c33 = cfmt("%.$(nflts)f",loc_data.Req_C33[1]*10)*"\\times10^{9}";
      ax1 = Axis(f[1, 3], yticklabelcolor = colors[1], ylabelcolor = colors[1], xlabel = L"\text{Iteration}", ylabel=L"$\bar{d}_h(\Omega)$",
        xtickalign=1,ytickalign=1,xminorticksvisible=true,yminorticksvisible=true,xgridvisible=true,ygridvisible=true,
        xminortickalign=1,yminortickalign=1,xminorticks=IntervalsBetween(5),yminorticks=IntervalsBetween(5),ytickformat = "{:.2f}", width = plots_size, height = plots_size)
      if ylims[1] == :manual
        ylims!(ax1,ylims[2],ylims[3])
      end
      ax2 = Axis(f[1, 3], yticklabelcolor = colors[2], ylabelcolor = colors[2], yaxisposition = :right, ylabel=L"""\bar{C}_{zzzz}(\Omega)-%$(req_c33)""",
        xtickalign=1,ytickalign=1,xminorticksvisible=true,yminorticksvisible=true,xgridvisible=true,ygridvisible=false,
        xminortickalign=1,yminortickalign=1,xminorticks=IntervalsBetween(5),yminorticks=IntervalsBetween(5),ytickformat = "{:.2f}", width = plots_size, height = plots_size,)
      hidespines!(ax2)
      hidexdecorations!(ax2)
