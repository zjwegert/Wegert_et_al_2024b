...
## Aniso
# d1 = data[data.Initial_struc .== "Regular" .&& data.Objective .== "Bh_Voigt",:]
# d2 = data[data.Initial_struc .== "Regular" .&& data.Objective .== "Bh_Reuss",:]

# using Gridap, Gridap.TensorValues

# function elastic_aniso(d)
#   C  = d.C
#   κ = (C[1,1] + C[2,2] + C[3,3] + 2*(C[1,2] + C[1,3] + C[2,3]))/9
#   μ = (C[1,1] + C[2,2] + C[3,3] - (C[1,2] + C[1,3] + C[2,3]) + 3*(C[4,4] + C[5,5] + C[6,6]))/15;
#   λ = κ - 2/3*μ
#   Ciso = [
#     λ+2μ λ λ 0 0 0
#     λ λ+2μ λ 0 0 0
...
#     0 0 0 0 0 μ
#   ]
#   Cijkl = SymFourthOrderTensorValue(C[1,1], C[6,1], C[5,1], C[2,1], C[4,1], C[3,1],
...
#   )
#   Ciso_ijkl = SymFourthOrderTensorValue(Ciso[1,1], Ciso[6,1], Ciso[5,1], Ciso[2,1], Ciso[4,1], Ciso[3,1],
#     Ciso[1,6], Ciso[6,6], Ciso[5,6], Ciso[2,6], Ciso[4,6], Ciso[3,6],
#     Ciso[1,5], Ciso[6,5], Ciso[5,5], Ciso[2,5], Ciso[4,5], Ciso[3,5],
#     Ciso[1,2], Ciso[6,2], Ciso[5,2], Ciso[2,2], Ciso[4,2], Ciso[3,2],
#     Ciso[1,4], Ciso[6,4], Ciso[5,4], Ciso[2,4], Ciso[4,4], Ciso[3,4],
#     Ciso[1,3], Ciso[6,3], Ciso[5,3], Ciso[2,3], Ciso[4,3], Ciso[3,3]
#   )
#   sqrt(((Cijkl - Ciso_ijkl) ⊙ (Cijkl - Ciso_ijkl))/(Ciso_ijkl ⊙ Ciso_ijkl))
# end

# plot(map(aniso,eachrow(d1)))
# plot(abs.(d1.Bh_Reuss-d1.Bh_Voigt)./d1.Bh_Voigt)
# plot(map(aniso,eachrow(d2)))
# plot(abs.(d2.Bh_Reuss-d2.Bh_Voigt)./d2.Bh_Voigt)

# f,ax,plt=scatter(map(x->x[3,3],d2.C))
# scatter!(ax,map(x->x[2,2],d2.C))
# f